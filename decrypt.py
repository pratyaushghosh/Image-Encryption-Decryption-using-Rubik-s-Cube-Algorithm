...
def inputImage():
	im = Image.open('encrypted_images/encrypted_result.png')
	pix = im.load()
	return im, pix
...
def inputKeys(im):
	...
	with open("Kr.txt") as kr:
		for line in kr:
			Kr.append(int(line))

	with open("Kc.txt") as kc:
		for line in kc:
			Kc.append(int(line))
...
def decryptImage(im, pix, m, n, r, g, b, Kr, Kc, ITER_MAX):
	...
	im.save('decrypted_images/decrypted_result.png')
	print('Decrypted')

#main
im, pix = inputImage()
r, g, b = rgbMatrix(im, pix)
m, n, Kr, Kc, ITER_MAX = inputKeys(im)
decryptImage(im, pix, m, n, r, g, b, Kr, Kc,  ITER_MAX)
